...
//!
//! CASM names permit `-` and `.`, which are not legal in TLA+ identifiers. TLA+ has
...
fn write_topology(
    ...
    let _ = writeln!(
        out,
        "(* The transitive closure of BlockingDeps, computed by CASM"
    );
...
fn emit_config(blast_radius: usize, max_failures: usize) -> String {
    ...
    let _ = writeln!(config, "\\* Generated by CASM -- safety properties.");
...
fn emit_liveness_config(blast_radius: usize, max_failures: usize) -> String {
    let mut config = String::new();
    let _ = writeln!(config, "\\* Generated by CASM -- liveness properties.");
    let _ = writeln!(
        config,
