...
[release archive](https://github.com/casimirex/casm/releases) — every archive carries
both `casm` and `casm-lsp`.

`casm-lsp` speaks the Language Server Protocol, so VS Code, Neovim, Helix, and Zed all
work. Setup for each is in [`editors/`](https://github.com/casimirex/casm/tree/main/editors).
